...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	public SecurityConfig(ProjectManagerDetailsService projectManagerDetailsService,
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable().authorizeRequests()
				.antMatchers("/auth/health-check", "/auth/authenticate", "/auth/validate").permitAll().anyRequest()
				.authenticated().and().sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS); // stopping
																													// default
																													// session
																													// creation
